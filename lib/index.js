...
const debug = (0, _debug2.default)('koa-acl-swagger:main');
...
class AclSwagger extends _acl2.default {
  ...
  middleware_koa(ctx, next) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const resource = _this.getResource(url, method, parmas);
      debug(`getResource url: ${ url } method: ${ method } resource: ${ resource }`);
      if (resource === null) return ctx.throw(_this.ERROR.UNKNOWN_PATH, 404);
      if (!resource === '') // no permission need
        return next();

      const userId = typeof _this.getUserId === 'function' ? yield _this.getUserId(ctx, next) : ctx.session && ctx.session.userId || null;
  ...
  getResource(url, method, params) {
    ...
    if (paths.hasOwnProperty(url)) return _getResource(paths, url, method);

    const pathsArr = Object.keys(paths);
    for (let path of pathsArr) {
      if (this._openAPIFormat(path, params) === url) return _getResource(paths, path, method);
    }
  ...
  _openAPIFormat(url, obj) {
    ...
    arr.forEach(function (value, index) {
      if (value.match(/^\{\w+\}$/g)) {
        ...
        if (obj.hasOwnProperty(value)) arr[index] = obj[value];
      }
    });
